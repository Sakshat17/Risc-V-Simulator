...
def hazardDetectionUnitControlPrev():
    global isDataHazard, prevIns, prevPrevIns, prevPrevPrevIns, curIns, dataHazzardStalls, dataHazzards, numberOfStalls, isDependentPrev, isControlStall, MuxRASelect, stallCount, MuxRBSelect, controlEDType, controlHazzards, controlHazzardStalls
    isDependentPrev = False
    if (rs1 != ""):
        if (rs1 == prevIns[1] and rs1 != "00000"):
            isDataHazard = 1
...
def MuxRA():
    global RA, MuxRASelect, RZ, RY, pRZ
    if (MuxRASelect == 0):
        RA = registerFile[int(rs1, 2)]
    elif (MuxRASelect == 1):
        RA = pRZ[:]
